...
export const DDBB = () => {
  ...
  const getData = async () => {
    try {
      const res = await axios.get('linkpeticion',
        {
    ...
    try {
      if (op === 1) {
        if ((nombre !== '') && (numero !== '') && (modelo !== '') && (local !== '') && (numero.length === 10)) {
          ...
          await axios.post('linkpeticion', newData, {
            withCredentials: true,
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`,
        ...
        await axios.put(`linkpeticion/${id}`, newData, {
          withCredentials: true,
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const deleteItem = async (id) => {
    try {
      await axios.delete(`linkpeticion/${idDelete}`, {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
